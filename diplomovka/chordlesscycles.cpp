 ...
 */
shared_ptr < vector < Cycle > > chordless_cycles(const Graph &G) {
    ...
    for (int i = 0; i < G.noOfVerticies; i++) {
        ...
        for (int j = 1; j < 3; j++) {
            for (int k = 0; k < j; k++) {
                ...
                Cycle cyklus;
                cyklus.push_back(i);
                cyklus.push_back(firstV.neighbors[j]);
                if (adjacent(G.vertices[firstV.neighbors[j]],
                             G.vertices[firstV.neighbors[k]])) {
                    //Cycle of length 3
                    cyklus.push_back(firstV.neighbors[k]);
                    cycles->push_back(cyklus);
                    continue;
                }

                vector< Cycle > candidates;
                candidates.push_back(cyklus);
